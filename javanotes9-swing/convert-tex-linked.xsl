...
<xsl:template match="sourceref"><xsl:variable name="a" select="str:new(@href)"/><xsl:variable name="b" select="str:replaceAll($a,'_','\\_')"/>
   ...
   <xsl:variable name="ref">
      <xsl:choose>
         <xsl:when test="ancestor::source"><xsl:text>http://math.hws.edu/eck/cs124/javanotes9-swing/source/</xsl:text><xsl:value-of select="@href"/></xsl:when>
         <xsl:otherwise><xsl:text>http://math.hws.edu/eck/cs124/javanotes9-swing/source/chapter</xsl:text><xsl:value-of select="$chapternum"/><xsl:text>/</xsl:text><xsl:value-of select="@href"/></xsl:otherwise>
      </xsl:choose>
   </xsl:variable>
   <xsl:choose>

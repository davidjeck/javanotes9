...
if  $XALAN_COMMAND -xsl convert-epub.xsl -in javanotes7-epub.xml ; then

   if [ ! -e "epub/OEBPS/javanotes7.opf" ] ; then
      echo Some error occurred while running xalan; epub/OEBPS/javanotes7.opf not created
      exit 1
   fi
   ...
   cp javanotes7-cover-518x675.jpg epub/OEBPS
   ...
   cd epub
   zip -X -Z store javanotes7.zip mimetype
   zip -Z store -r javanotes7.zip META-INF
   zip -r javanotes7.zip OEBPS
   cd ..

   if ! mv epub/javanotes7.zip $BUILD_OUTPUT_DIR/javanotes7.epub ; then
   ...
   if [ -x "$KINDLEGEN" ]; then
      ...
      echo
      cd $BUILD_OUTPUT_DIR
      $KINDLEGEN javanotes7.epub 
      if [ -f "javanotes7.mobi" ]; then
         echo
